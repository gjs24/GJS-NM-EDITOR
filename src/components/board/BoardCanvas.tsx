...
import { BoardTemplate, EditableField, UserBoardValues } from '../../types/template';
  ...
  useEffect(() => {
    const updateScale = () => {
      if (containerRef.current) {
        ...
        // Leave generous room for header (50px), footer hint (40px), and top/bottom padding (60px)
        const maxW = Math.max(300, Math.min(availableWidth - 64, 1000));
        const maxH = Math.max(260, availableHeight - 150);

        const scaleW = maxW / template.baseWidth;
        const scaleH = maxH / template.baseHeight;
        // Use the smaller scale so that BOTH width and height fit completely without overflow!
